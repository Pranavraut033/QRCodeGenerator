...
public final class Encoder {
    ...
    public Bitmap encodeAsBitmap() throws WriterException {
        ...
        int scaleSize;
        scaleSize = dimension / width;
        if (scaleSize > dimension / height) scaleSize = dimension / height;
        int[] pixels = new int[dimension * dimension + 4 * padding];

        for (int y = padding; y < height; y++) {
            int offset = y * dimension * scaleSize;
            for (int psH = padding; psH < scaleSize; psH++, offset += dimension)
                for (int x = 0; x < width; x++) {
                    int color = result.get(x, y) ? foreground : background;
                    for (int psW = 0; psW < scaleSize; psW++)
                        pixels[offset + x * scaleSize + psW] = color;
                }
        }
    ...
    public Encoder setBackground(int background) {
        this.background = background;
        return this;
    }

    public Encoder setForeground(int foreground) {
