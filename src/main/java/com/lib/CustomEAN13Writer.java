...
//
     ...
     * @return true if string of digits passes the UPC/EAN checksum algorithm
    ...
    public int appendPatternAndConsiderLongLinePosition(boolean[] target, int pos, int[] pattern,
        ...
        for (int len : pattern) {
            for (int j = 0; j < len; j++) {
                ...
                if (Arrays.equals(pattern, START_END_PATTERN) || Arrays.equals(pattern, MIDDLE_PATTERN)
                        || Arrays.equals(pattern, START_END_PATTERN))
